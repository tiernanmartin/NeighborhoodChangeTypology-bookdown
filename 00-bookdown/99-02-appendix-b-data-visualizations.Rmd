...
The following plots display a lot of data in a visually-dense format. The indicators for each COO Community are arranged in a column, with each grid square containing the following information:
  ...
  * the indicator values for each census tract (represented by a thin vertical line)
  * the indicator values for all census tracts (represented by a light gray histogram chart in the grid's background)
  * the indicator value for King County (represented by a thick, blue, vertical line)
  * _for some indicators only:_ the indicator value of all of the tracts in the COO community combined (represented by a thick dashed vertical line)
  
Reading the plot one column at a time, from top to bottom, the view can see how each COO communities' tracts compare to the Count and the rest of the county's tracts.
